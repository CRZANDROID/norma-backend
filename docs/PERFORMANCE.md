...
- **`failed` / `stalled`** — se rompió, o BullMQ creyó que el crawl se colgó. El worker de crawl usa `lockDuration` ~15 min + `lockRenewTime` 15 s. En esta versión de BullMQ `stalled` no es un tipo de job: sale `0` en `/jobs/status`. Se detecta si el mismo crawl se relanza o `failed` sube mientras progress sigue en `running`.
